...
var _ = Describe("policy_route_test", func() {
	...
	It("policy_route : test system policy route", func() {
		...
		cidr, err := utils.NetmaskToCIDR(utils.AdditionalPubNicsForUT[0].Netmask);utils.PanicOnError(err)
		addr1 := fmt.Sprintf("%v/%v", utils.AdditionalPubNicsForUT[0].Ip, cidr)
		_, cidr1, _ := net.ParseCIDR(addr1)
		cidr, err = utils.NetmaskToCIDR(utils.PrivateNicsForUT[0].Netmask);utils.PanicOnError(err)
		addr3 := fmt.Sprintf("%v/%v", utils.PrivateNicsForUT[0].Ip, cidr)
		_, cidr3, _ := net.ParseCIDR(addr3)
		cidr, err = utils.NetmaskToCIDR(utils.PrivateNicsForUT[1].Netmask);utils.PanicOnError(err)
