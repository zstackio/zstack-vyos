...
var VYOSHA_DNSMASQ_SCRIPT = filepath.Join(utils.GetZvrRootPath(), "keepalived/script/dnsmasq.sh")
...
func writeDnsmasqHaScriptForVyos() {
	...
	if exist, _ := utils.PathExists(VYOSHA_DNSMASQ_SCRIPT); exist {
		return
	}

	var content = fmt.Sprintf("pkill -9 dnsmasq; %s", DNSMASQ_BIN_PATH)
	err := ioutil.WriteFile(VYOSHA_DNSMASQ_SCRIPT, []byte(content), 0755)
	utils.PanicOnError(err)
}
