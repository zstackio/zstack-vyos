...
const (
	...
	zstackRuleNumberEnd = 9000
	USER_RULE_SET_DES_PREFIX = "ZStack-User-RuleSet"
)
...
func (r *ruleSetInfo) toRules() []string {
	...
	if r.Description != "" {
		rules = append(rules, fmt.Sprintf("description %s", r.Description))
	}
...
func deleteRule(ctx *server.CommandContext) interface{} {
	...
	rule := cmd.Rule
	tree := server.NewParserFromShowConfiguration().Tree
	tree.Deletef("firewall name %s rule %s", rule.RuleSetName, rule.RuleNumber)
	tree.Apply(false)
	return nil
}
...
func getFirewallConfig(ctx *server.CommandContext) interface{} {
	...
	if ruleSetNodes := rs.Children(); ruleSetNodes == nil {
		...
		for _, ruleSetNode := range ruleSetNodes {
			...
			if d := ruleSetNode.Get("description"); d != nil {
				ruleSet.Description = d.Value()
			}
			...
			if r := ruleSetNode.Get("rule"); r != nil {
				for _, rc := range r.Children() {
					rules = append(rules, ruleInfo{
						...
						Description: rc.GetChildrenValue("description"),
	...
	for _,e := range ethInfos {
		eNode := tree.Getf("interfaces ethernet %s firewall", e.Name)
...
func deleteOldRules(tree *server.VyosConfigTree) {
	...
	if ruleSetNodes := rs.Children(); ruleSetNodes != nil {
		for _, ruleSetNode := range ruleSetNodes {
			if d := ruleSetNode.Get("description"); d != nil {
				des := d.Value()
				if strings.HasPrefix(des, USER_RULE_SET_DES_PREFIX) {
