...
func (r *ruleInfo) toGroups(suffix string) []string {
	...
	for _, ip := range(ips)  {
		if _, cidr, err := net.ParseCIDR(strings.TrimSpace(ip)); err == nil {
			min := cidr.IP
			max := utils.GetUpperIp(*cidr)
                        log.Debugf("ip range:%s-%s in %s", min.String(), max.String(), ip)
			groups = append(groups, fmt.Sprintf("%s-%s", min.String(), max.String()))
		} else {
			groups = append(groups, fmt.Sprintf("%s", strings.TrimSpace(ip)))
		}
	}
	return groups
}
