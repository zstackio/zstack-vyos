...
func configureNtp(timeServers []string){
	...
	buf.WriteString(`# /etc/ntp.conf, configuration for ntpd; see ntp.conf(5) for help
...
`)
	...
	for _,chronyServer := range timeServers {
		buf.WriteString("server " + chronyServer + "\n")
	}


	ntp_conf_file, err := ioutil.TempFile(NTP_CONF_DIR, "ntpConfig"); utils.PanicOnError(err)
	_, err = ntp_conf_file.Write([]byte(buf.String())); utils.PanicOnError(err)
