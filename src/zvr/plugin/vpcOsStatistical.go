...
func (c *vpcOsCollector) Update(ch chan<- prom.Metric) error {
	...
	if memInfo != nil {
		...
		ch <- prom.MustNewConstMetric(c.vpcMemoryUsage, prom.GaugeValue, float64((memInfo.kb_Memtotal-kb_mem_available)*1024), "used")
...
func NewVpcOsPrometheusCollector() MetricCollector {
	return &vpcOsCollector{
		...
		vpcDiskUsage: prom.NewDesc(
			...
			[]string{LABEL_FILESYSTEM_DEVOCE, LABEL_FILESYSTEM_FSTYPE, LABEL_FILESYSTEM_MOUNTPOINT, LABEL_FILESYSTEM_TYPE}, nil,
		),
	}
}
...
func getVPCCpuInfo() []*cpuInfo {
	...
	if err != nil {
		log.Error("read /proc/stat failed: %v", err)
		return nil
	}
	stdout := string(infoFromFile)
...
func getDiskCpuInfo() []*diskInfo {
	bash := &utils.Bash{
		Command: fmt.Sprintf("df -h -l -B 1 | grep vda1"),
		NoLog:   true,
	}
